...
export function FeaturedProductHero() {
    ...
    return (
        <div className="w-full h-full bg-white flex items-center justify-center p-2 sm:p-4 lg:p-6 overflow-hidden">
            <div className="grid md:grid-cols-2 gap-4 lg:gap-6 items-center w-full h-full max-w-6xl mx-auto">

                {/* Left side: Text & CTA */}
                <div className="flex flex-col items-start pt-4 sm:pt-10 pb-4">
                    <div className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 text-indigo-700 text-[10px] sm:text-xs font-bold rounded-full mb-3 sm:mb-4 relative group border border-indigo-100/50">
                        <ShieldCheck className="w-3.5 h-3.5" />
                        {featuredProductHero.badgeLabel || 'FSSAI & NSF Certified'}
                    </div>

                    <p className="text-gray-900 font-bold mb-2">HyperWhey Pro</p>

                    <h1 className="text-2xl sm:text-3xl lg:text-4xl font-black text-gray-900 leading-[1.1] mb-1 sm:mb-2 tracking-tight">
                        {featuredProductHero.heading || "World's Best Whey Protein"}
                    </h1>

                    <p className="text-base sm:text-lg lg:text-xl text-[#1a237e] font-bold mb-2 sm:mb-3">
                        {featuredProductHero.subheading || "Now in India"}
                    </p>

                    <p className="text-xs sm:text-sm text-gray-500 mb-4 sm:mb-6 font-medium">
                        27g Protein · Zero Sugar · 30 Servings · Mixes Instantly
                    </p>

                    <div className="flex flex-wrap items-center gap-3 mb-4 sm:mb-6">
                        <Link
                            href="/product/1"
                            className="bg-[#1a237e] hover:bg-[#0d1459] text-white px-6 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 transition-all shadow-lg shadow-indigo-200"
                        >
                            Shop Now — ₹2,499 <ArrowRight className="w-4 h-4 ml-1" />
                        </Link>

                        <div className="bg-[#e53935] text-white px-2.5 py-1 rounded-lg font-black text-xs tracking-wide">
                            24% OFF
                        </div>
                    </div>
                ...
                <div className="relative h-48 sm:h-full min-h-[160px] w-full rounded-2xl bg-[#f4f7ff] flex items-center justify-center p-2 sm:p-4 group">

                    {/* Rating Badge (Top Right) */}
                    <div className="absolute top-6 right-6 bg-white shadow-xl shadow-blue-900/5 rounded-xl p-3 flex flex-col items-center gap-0.5 z-10 border border-gray-100">
                    ...
                    <div className="absolute bottom-6 left-6 bg-white shadow-xl shadow-blue-900/5 rounded-xl px-4 py-3 flex items-center gap-3 z-10 border border-gray-100">
